...
def seed_data(cur):
    ...
    # 3. Finance (Expenses)
    # Check if we have *any* transactions, if not, seed some defaults
    ...
    print("✅ Data seeding check complete.")

def init_db():
    """
    Main entry point to initialize the database.
    """
    ...
    try:
        ...
        # We might not want to crash the whole agent if DB init fails,
        # but it likely means the agent won't work anyway.
        # For now, we log and continue (or re-raise if strict).
